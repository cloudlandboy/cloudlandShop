...
var ckmem =function(isCk){
        ...
        $.post("/ajax/verifmobile/",{"name":"userMobile","param":val},function(data){
            if(data!="y"){
                if(data.indexOf("请直接登录")>=0){
                    ...
                    if(data.indexOf("云梦速运")>-1){
                        newData = "该手机在云梦速运已注册，请<a href='/' style='color:#669900'>直接登录></a>或<a href='/reg/findpass/?returnUrl=http://www.cloudlandshop.cn/' target='_blank' style='color:#669900'>找回密码></a>";
                    }
    ...
    /*else {
     ...
     var newData = comMsg(data).replace("请直接登录","<a href='/' style='color:#669900'>请直接登录</a>");
     if(data.indexOf("云梦速运")>-1){
     newData = "该邮箱在云梦速运已注册，请<a href='/' style='color:#669900'>直接登录></a>或<a href='/reg/findpass/?returnUrl=http://www.cloudlandshop.cn/' target='_blank' style='color:#669900'>找回密码></a>";
     }
     $("#"+MailMId).parent().attr("class","").addClass("regM errorBorder").next().attr("class","");
     $("#"+MailMErrId).attr("class","reg_error").html(newData);
     }else{
    ...
    $.post("/ajax/verifmobile/",{"name":"userMobile","param":cpymobile},function(data){
        if(data!="y"){
            if(data.indexOf("请直接登录")>=0){
                ...
                var msg_new =comMsg(data).replace("请直接登录","<a href='/' style='color:#669900'>请直接登录</a>");
                if(data.indexOf("云梦速运")>-1){
                    msg_new = comMsg("该手机在云梦速运已注册，请<a href='/' style='color:#669900'>直接登录></a>或<a href='/reg/findpass/?returnUrl=http://www.cloudlandshop.cn/' target='_blank' style='color:#669900'>找回密码></a>");
                }
    ...
    $.post("/ajax/verifemails/",{"name":"outemail","param":cpymail},function(data){
        if(data!="y"){
            if(data.indexOf("请直接登录")>=0){
                ...
                var msg_new =comMsg(data).replace("请直接登录","<a href='/' style='color:#669900'>请直接登录</a>");
                if(data.indexOf("云梦速运")>-1){
                    msg_new = comMsg("该邮箱在云梦速运已注册，请<a href='/' style='color:#669900'>直接登录></a>或<a href='/reg/findpass/?returnUrl=http://www.cloudlandshop.cn/' target='_blank' style='color:#669900'>找回密码></a>");
                }
